...
import { markPresentByTransaction } from '../attendance';
...
/**
 * Structural stand-ins for `apps/library-api`'s `IssueBookDto` /
 * `ReturnBookDto`. Those are class-validator classes and belong to the HTTP
 * layer — `@library/core` must not depend on the decorators, and a second
 * caller (`apps/api`'s librarian counter) will arrive here with a plain
 ...
 *
 * Concurrency: this function does NOT itself serialize two callers issuing
 ...
 * this function turns into a 409 rather than an unhandled 500. An
 ...
 * function — the index, not the interceptor, is the actual guarantee (see
 * issue-constraints.e2e.spec.ts for the proof, and the circulation module's own
...
export async function issue(
  ...
  try {
    ...
    // issue_one_active_per_copy — see this function's own concurrency doc
...
export async function returnBook(
  ...
  const { days, amount } = computeFine(policy, activeIssue.dueAt, now);
  ...
  const promotion = await promoteOrRelease(tx, orgId, copy.id, copy.branchId, copy.titleId, actorUserId, now);
 ...
 * the copy actually being returned right now (see that function's doc).
 *
 ...
 */
async function sweepExpiredReadyHolds(tx: LibraryTx, orgId: string, titleId: string, actorUserId: string, now: Date): Promise<void> {
 ...
 * in `returnBook` — so RLS still applies to it exactly as it would to a
 * generated query.
 */
async function promoteOrRelease(
  tx: LibraryTx,
  orgId: string,
  copyId: string,
  branchId: string,
  ...
  await sweepExpiredReadyHolds(tx, orgId, titleId, actorUserId, now);
