/**
 * Every ISO 3166-1 alpha-2 country, generated once from ICU's English region
 * names (Node 22, 2026-09-26) and committed — a
 * dependency for a list that changes once a decade is not worth the surface.
 *
 * School.countryCode is one of these. Today India is the only country with a
 * festival catalogue, a salary rule book and a locale; the others are here so
 * a school can be created correctly now and configured when its country is
 * built out. Supranational / reserved codes (EU, UN, XK…) are left out.
 */
...
/** The country a new school gets when nothing is chosen — the only one fully built out. */
