...
18a. At 390 px, every sports screen: the page itself never scrolls sideways — the timetable, the bracket and the day grid each scroll inside their own box, and every table sits in a wrapper that does. The day picker keeps ‹, the day list and › on one row whatever the longest option says. The house picker on Houses shows a full house name, not "Raff". Guarded mechanically by `app/app/sports/sports-responsive.test.ts`.
19a. The cost line travels: with a sport picked and players in, shorten the day so the plan needs two days, then move between The meet, Sports, Players and Review — the "2 needed · 1 booked" line and its "Make it 2 days" button are on all four. Press it from Review and the last day really moves on step 1.
...
20. Reading the meet: the tournament opens on **Schedule**, one shut row per sport carrying its count, venues and dates. Open the 100 m → its classes; open Class 9 → the heats with times. **Timetable**: the day picker is chips for a 2-day meet; make a meet whose plan spills over fifty days and it becomes "Day 1 of N" with a stepper and a list — never a strip of fifty chips, and the overrun is stated once above the views, not on every day. The grid gives a column only to venues with something on that day, names the rest, and "Show all venues" brings them back; a 5-minute heat is one readable line, a 25-minute match is five times taller.
21. Running the meet: on **Programme**, press Sport / Category / Day and watch the same rows re-hang. Open a sport → a category → a class; press **Move…** → "30 min later" and the whole class moves, with **Put … back** in the card head undoing it. A class whose slots are all played shows "all played" and no Move. Press **Runs on…** on a sport → a day → every event under it is held; "Wherever it fits" frees them. On **Timetable**, drag a heat onto a busy court → refused with the slot already there named; drag it onto a free gap → it moves; try to drag a green (played) block → refused without a network call. Press **Clear** on a venue heading → everything unplayed comes off it and re-lays; the last venue for an event refuses by name.
22. Where this meet is: the panel above the views lists what is done, blocked and next. Give an event no venue → "1 event has nowhere to play" in red with a Days & courts button that switches tab. Make a plan that needs 20 days → "Book 20 days" works, and the notice shows the arithmetic: hours on the busiest venue, hours in a day, days that venue alone needs, and which venues hold nothing. Book a month → Days & courts lists only the days holding something, with "N booked days hold nothing" and a control to show all. Type 2126 as the last day → refused with "check the year on the last day". Clear everything → "Nothing is in the way." and Publish moves to Next.
23. The two gaps: set **Break between slots on a court** to 10 and lay the plan out again → consecutive slots on one court are 10 minutes apart; **Rest between a child's own slots** still only spaces one child's own events. The cost line and the day bars both count the break.
24. Brackets: a padded draw shows byes as one muted line with "N byes" on the round label, and connector lines join each pair to the card it feeds.
25. Days & courts (on a created meet): the tab shows a row per day with a bar per court and the events that run; a meet whose plan spilled shows the extra day marked "not booked" and the Day board carries the same warning. Press "Book N days" → the tab redraws with the warning gone and no played slot has moved. Add "Court 2" → every event of that type gains it and the plan re-lays. Hold Chess to day 1 → its slots move to that day; set it back to "Wherever it fits" → it spreads again. On a LIVE meet the hours and the rest gap are disabled while the last day and Add venue still work; on a DONE meet every one of them answers 409.
