...
18. Step 3: "Enter class 9" fills a column; a child ticked into four events is badged; Football's panel suggests a basis with the team count on each chip. Enter only one class with one section → "Only 1 team under sections" with the working fix (or the message to tick more children). Create and open the board: no child appears twice at the same minute, and the Clashes view is empty.
