...
| "Chess picked up the football field" | Never designed: venues bind by the sport's own type, or a venue named after it; a sport with no matching venue gets NONE and the line says which type to add | Any event pre-selecting a venue of the wrong type |
| "Table tennis has no venue" | The school named no table; rename a court to "TT table", add one, or tick a venue on the line with Edit | A table in the venues not bound to table tennis |
| "One class for football is refused" | A team sport needs two teams: under sections one class with one section is one team. The panel says which basis WOULD work and switches with one press | The suggested basis not offered, or a basis with two teams still refused |
