/**
 * Which routes have had their authorization looked at.
 *
 * Read `route-coverage.e2e-spec.ts` for why this file exists. In short: a
 * controller shipped with a guard on two of its six handlers and a STUDENT
 * token could delete staff records, and no test noticed for months. This
 * manifest makes "nobody has looked at this route" a failing test rather than
 * a silent state.
 *
 * It is NOT a claim that a route is correctly guarded — no static list can be.
 * It is a claim that a human decided who may call it and wrote an assertion.
 */

/**
 * Routes whose authorization has been decided AND asserted in a test.
 *
 * Add a route here only once there is an assertion covering it — otherwise
 * this becomes a list of good intentions.
 */
export const AUTHZ_REVIEWED: string[] = [
  // ── The Press ───────────────────────────────────────────────────────────
  // press-authz.e2e-spec.ts: every office route refuses anonymous / STUDENT /
  // TEACHER and admits SCHOOL_ADMIN + STAFF; the portal routes are proven
  // caller's-own-JWT (another family's card answers 404, asserted with two
  // seeded students); and the PRESS override (no tier grants it) gates the
  // whole module — a PRO school without it 403s even as an admin.
  // management-authz.e2e-spec.ts "the Morning Bell": admin reads it; teacher,
  // staff, student and anonymous are each refused.
  "GET /manage/bell",
  // management-authz "the Front Desk": admin passes; teacher/staff/student/
  // anonymous each refused, for all three reads.
  "GET /manage/search",
  "GET /manage/pulse",
  "GET /manage/students/:id/report",
  // tv.e2e-spec.ts: the public screen is gated ONLY by the display key (off,
  // wrong-key and not-live all answer the same 404); the switch is
  // SCHOOL_ADMIN-only, proven against anonymous/student/teacher/staff; a
  // rotate invalidates the old key and disable kills every link.
  "GET /public/tv",
  "GET /manage/tv",
  "GET /manage/tv/rotate",
  "GET /manage/tv/disable",
  "GET /manage/press/overview",
  // The Result Room — press-authz OFFICE_ROUTES (admin+staff in, teacher/
  // student/anon refused); the gate's 409 + override is unit-proven.
  "GET /manage/press/results",
  "POST /manage/press/results/nudge",
  "POST /manage/press/results/generate",
  // Teacher-visible result-day banner — management-authz asserts TEACHER and
  // SCHOOL_ADMIN pass, STUDENT and anonymous are refused.
  "GET /manage/exams/result-days",
  "POST /manage/press/certificates/bulk",
  "GET /manage/press/years",
  "GET /manage/press/classes",
  "GET /manage/press/students",
  "GET /manage/press/windows",
  "PUT /manage/press/windows",
  "GET /manage/press/report-cards/:windowId/:classSectionId",
  "PUT /manage/press/remarks",
  "POST /manage/press/report-cards/issue",
  "GET /manage/press/certificates/prepare/:studentId",
  "POST /manage/press/certificates/issue",
  "GET /manage/press/register",
  "GET /manage/press/register/:id",
  "POST /manage/press/register/:id/void",
  // Press Orders — press-authz.e2e-spec.ts. The school's order counter runs
  // the same office wall as the rest of the Press (and the same PRESS gate,
  // asserted separately so a new controller cannot mean a forgotten gate);
  // the operator desk routes are owner-host + platform-JWT, proven four ways
  // each (anonymous, school token on owner host, platform token on school
  // host, operator through); the lifecycle test proves another school's
  // admin reads 404 on a foreign order.
  "GET /manage/press/orders",
  "GET /manage/press/orders/:id",
  "POST /manage/press/orders/report-cards",
  "POST /manage/press/orders/upload",
  "POST /manage/press/orders/:id/confirm",
  "POST /manage/press/orders/:id/cancel",
  "GET /owner/print-orders",
  "GET /owner/print-orders/:id",
  "GET /owner/print-orders/:id/artifact",
  "POST /owner/print-orders/:id/quote",
  "POST /owner/print-orders/:id/decline",
  "POST /owner/print-orders/:id/printing",
  "POST /owner/print-orders/:id/dispatch",
  "POST /owner/print-orders/:id/delivered",
  "GET /me/report-cards",
  "GET /me/report-cards/:id",
  // ── Homecoming / the alumnus's own door ─────────────────────────────────
  // homecoming-portal-authz.e2e-spec.ts proves the lines BETWEEN the three
  // tiers, not merely that each works: public batch pages readable with no
  // token, member routes refusing anonymous / forged / SCHOOL_ADMIN-JWT
  // callers, and trusted routes refusing a VERIFIED but untrusted alumnus.
  "POST /manage/alumni/:id/claim-link",
  "GET /alumni/batches",
  "GET /alumni/batches/:year",
  "POST /alumni/claim",
  "POST /alumni/login",
  "POST /alumni/link-request",
  "POST /alumni/claims",
  "GET /alumni/me",
  "PUT /alumni/me",
  "PUT /alumni/me/password",
  "POST /alumni/me/sign-out",
  "GET /alumni/me/directory",
  "GET /alumni/me/gift-groups",
  "GET /alumni/me/gift-items",
  "POST /alumni/me/pledges",
  "GET /alumni/me/pledges",
  "GET /alumni/me/giving",
  "POST /alumni/me/pledges/:id/request-pickup",
  "POST /alumni/me/pledges/:id/picked-up",
  "GET /alumni/me/sessions/slots",
  "GET /alumni/me/sessions",
  "POST /alumni/me/sessions",
  "POST /alumni/me/sessions/:id/decide",
  // ── Homecoming / Alumni Office ──────────────────────────────────────────
  // Every one of these is exercised in homecoming-authz.e2e-spec.ts against
  // an anonymous caller (401) and STUDENT / TEACHER / STAFF tokens (403), and
  // is confirmed NOT to 401/403 for a SCHOOL_ADMIN. The feature gate and
  // cross-tenant isolation are asserted over the whole set as well.
  "GET /manage/alumni/summary",
  "GET /manage/alumni",
  "POST /manage/alumni/graduate",
  "GET /manage/alumni/roll-call",
  "PUT /manage/alumni/roll-call",
  "PUT /manage/alumni/:id/trusted",
  "GET /manage/alumni/claims",
  "POST /manage/alumni/claims/:id/decide",
  "POST /manage/alumni/:id/account",
  "GET /manage/alumni/link-requests",
  "POST /manage/alumni/link-requests/:id/sent",
  "POST /manage/alumni/link-requests/:id/dismiss",
  "GET /manage/alumni/gift-items",
  "POST /manage/alumni/gift-items",
  "PUT /manage/alumni/gift-items/:id",
  "GET /manage/alumni/gift-groups",
  "GET /manage/alumni/pledges",
  "POST /manage/alumni/pledges",
  "POST /manage/alumni/pledges/:id/decide",
  "POST /manage/alumni/pledges/:id/receive",
  "POST /manage/alumni/pledges/:id/distribute",
  "POST /manage/alumni/pledges/:id/report",
  "POST /manage/alumni/pledges/:id/request-pickup",
  "POST /manage/alumni/pledges/:id/picked-up",
  "POST /manage/alumni/pledges/:id/purchase",
  "POST /manage/alumni/pledges/:id/thank-you",
  "POST /manage/alumni/pledges/:id/attachments",
  "DELETE /manage/alumni/pledges/:id/attachments/:attachmentId",
  "GET /manage/alumni/slots",
  "GET /manage/alumni/sessions",
  "POST /manage/alumni/sessions",
  "GET /manage/alumni/sessions/:id/conflicts",
  "POST /manage/alumni/sessions/:id/decide",
  "POST /manage/alumni/sessions/:id/decide-as-host",
  // Fees. Every route below is asserted in fees-authz.e2e-spec.ts — anonymous,
  // wrong-role and right-role, plus a feature-gate check. 147 assertions.
  "DELETE /manage/fees/categories/:id",
  "DELETE /manage/fees/concessions/:id",
  "GET /manage/fees/billing/preview",
  "GET /manage/fees/categories",
  "GET /manage/fees/concessions",
  "GET /manage/fees/grid",
  "GET /manage/fees/payment-setup",
  "GET /manage/fees/payments",
  "GET /manage/fees/settings",
  "GET /manage/fees/students",
  "GET /manage/fees/students/:id",
  "GET /manage/fees/summary",
  "GET /manage/fees/terms",
  "GET /me/fees",
  "GET /me/fees/bank-instructions",
  "GET /me/fees/how-to-pay",
  "POST /manage/fees/billing/generate",
  "POST /manage/fees/categories/seed",
  "POST /manage/fees/concessions",
  "POST /manage/fees/payment-setup/bank/qr",
  "POST /manage/fees/payments/:id/reject",
  "POST /manage/fees/payments/:id/reverse",
  "POST /manage/fees/payments/:id/verify",
  "POST /manage/fees/payments/record",
  "POST /me/fees/submit",
  "PUT /manage/fees/categories",
  "PUT /manage/fees/grid",
  "PUT /manage/fees/payment-setup/bank",
  "PUT /manage/fees/payment-setup/provider",
  "PUT /manage/fees/settings",
  "PUT /manage/fees/terms",
  "DELETE /library/issues/:id",
  "GET /internal/cron/library-due-soon",
  "GET /library/dashboard",
  "GET /library/fines",
  "GET /library/hall",
  "GET /library/members",
  "GET /library/members/:kind/:id",
  "GET /library/settings",
  "GET /library/titles",
  "GET /library/titles/:id",
  "GET /me/library",
  "PATCH /library/settings",
  "POST /internal/cron/library-due-soon",
  "POST /library/fines/:id/collect",
  "POST /library/fines/:id/reopen",
  "POST /library/fines/:id/waive",
  "POST /library/fines/remind",
  "POST /library/hall/visits",
  "POST /library/issues",
  "POST /library/issues/:id/lost",
  "POST /library/issues/:id/reopen",
  "POST /library/issues/:id/return",
  "POST /library/issues/:id/unlose",
  "POST /library/titles",
  "POST /library/titles/:id/copies",
  "DELETE /manage/announcements/:id",
  "DELETE /manage/assignments/:id",
  "DELETE /manage/classes/:id",
  "DELETE /manage/staff/:id",
  "DELETE /manage/timetable/:id",
  "GET /internal/cron/notification-outbox",
  "GET /manage/announcements/mine",
  "GET /manage/assignments",
  "GET /manage/classes",
  "GET /manage/staff",
  "GET /manage/staff-attendance",
  "GET /manage/staff-attendance/mine",
  "GET /manage/timetable",
  "GET /manage/timetable/my-day",
  "GET /me/assignments",
  "PATCH /manage/announcements/:id",
  "POST /internal/cron/notification-outbox",
  "POST /manage/assignments",
  "POST /manage/assignments/upload",
  "POST /manage/classes",
  "POST /manage/staff",
  "POST /manage/staff/:id/invite/resend",
  "POST /manage/staff/:id/login",
  "POST /manage/timetable",
  "POST /me/assignments/:id/seen",
  "PUT /manage/classes/:id",
  "PUT /manage/staff-attendance",
  "PUT /manage/staff/:id",
  // Reviewed 27 Aug 2026, while adding the Homecoming routes — these 33 had
  // shipped into neither bucket, so this guard was red and nobody had looked.
  //
  // owner/*/domains  — OwnerController: OwnerHostGuard + PlatformJwtGuard. Owner
  //                    host required and a platform token; correct.
  // manage/rooms, manage/seating, manage/email-settings
  //                  — SchoolJwtGuard + RequireFeatureGuard + RolesGuard,
  //                    @Roles('SCHOOL_ADMIN') at class level; correct.
  // site/pages, site/design-drafts
  //                  — WERE SchoolJwtGuard alone, which reads no role: a STUDENT
  //                    token could publish to the live website. Fixed here, and
  //                    site-authoring-authz.e2e-spec.ts holds it shut.
  "GET /owner/schools/:id/domains",
  "POST /owner/schools/:id/domains",
  "POST /owner/schools/:id/domains/:domainId/verify",
  "POST /owner/schools/:id/domains/:domainId/primary",
  "DELETE /owner/schools/:id/domains/:domainId",
  "GET /site/design-drafts",
  "POST /site/design-drafts",
  "PUT /site/design-drafts/:id",
  "DELETE /site/design-drafts/:id",
  "POST /site/design-drafts/:id/publish",
  "GET /site/pages",
  "POST /site/pages",
  "PUT /site/pages/:id",
  "DELETE /site/pages/:id",
  "GET /manage/rooms",
  "POST /manage/rooms",
  "PUT /manage/rooms/:id",
  "POST /manage/rooms/:id/duplicate",
  "DELETE /manage/rooms/:id",
  "GET /manage/seating",
  "POST /manage/seating/preview",
  "POST /manage/seating",
  "GET /manage/seating/:id",
  "DELETE /manage/seating/:id",
  "GET /manage/email-settings",
  "GET /manage/email-settings/preview",
  "PUT /manage/email-settings",
  "PUT /manage/email-settings/sender",
  "POST /manage/email-settings/sender/verify",
  "POST /manage/email-settings/sender/disable",
  "POST /manage/email-settings/test",
  // Reviewed 27 Aug 2026. Every one of these carried SchoolJwtGuard ALONE,
  // which reads no role — so a STUDENT or PARENT token could edit the school's
  // public site, and /site/enquiries handed back other families' names and
  // phone numbers. All now RolesGuard + @Roles('SCHOOL_ADMIN'); every caller
  // already lives under /app, which is SCHOOL_ADMIN-only, so nothing that was
  // legitimately using them lost access.
  "DELETE /site/courses/:id",
  "DELETE /site/media/:id",
  "DELETE /site/staff/:id",
  "GET /site/admissions",
  "GET /site/content",
  "GET /site/courses",
  "GET /site/enquiries",
  "GET /site/hall-of-fame",
  "GET /site/media",
  "GET /site/staff",
  "PATCH /site/enquiries/:id",
  "POST /site/courses",
  "POST /site/media",
  "POST /site/staff",
  "PUT /site/admissions/settings",
  "PUT /site/admissions/steps",
  "PUT /site/courses/:id",
  "PUT /site/courses/:id/fee",
  "PUT /site/hall-of-fame/:courseId",
  "PUT /site/homepage",
  "PUT /site/profile",
  "PUT /site/social",
  "PUT /site/staff/:id",
  "PUT /site/stats",
  // Reviewed 27 Aug 2026, same defect and same fix as /site: a school token
  // with no role check. The public jobs BOARD is deliberately not here — it
  // lives on its own controller and stays reachable without a token.
  "DELETE /cms/blog/posts/:id",
  "DELETE /cms/blog/selections/:postId",
  "DELETE /manage/events/:id",
  "GET /cms/blog/library",
  "GET /cms/blog/posts",
  "GET /cms/blog/posts/:id",
  "GET /cms/blog/selections",
  "GET /cms/blog/settings",
  "GET /manage/events",
  "GET /manage/events/audience-candidates",
  "GET /manage/events/:id/registrations",
  "GET /manage/jobs",
  "GET /manage/jobs/:id/applications",
  "PATCH /cms/blog/posts/:id",
  "PATCH /cms/blog/selections/:postId",
  "PATCH /cms/blog/settings",
  "PATCH /manage/events/:id",
  "PATCH /manage/events/registrations/:registrationId",
  "PATCH /manage/jobs/:id",
  "PATCH /manage/jobs/applications/:id",
  "POST /cms/blog/posts",
  "POST /cms/blog/posts/:id/publish",
  "POST /cms/blog/posts/:id/submit-global",
  "POST /cms/blog/selections",
  "POST /manage/events",
  "POST /manage/events/:id/registrations",
  "POST /manage/jobs",
  "POST /manage/jobs/:id/close",
  "POST /manage/jobs/:id/submit",
  "DELETE /manage/grades/:id",
  "DELETE /manage/periods/:id",
  "DELETE /manage/subjects/:id",
  "DELETE /manage/teachers/:id",
  "GET /manage/availability",
  "GET /manage/grades",
  "GET /manage/periods",
  "GET /manage/subjects",
  "GET /manage/teachers",
  "GET /manage/teachers/me",
  "GET /manage/years",
  "POST /manage/grades",
  "POST /manage/periods",
  "POST /manage/subjects",
  "POST /manage/teachers",
  "POST /manage/teachers/:id/invite/resend",
  "POST /manage/teachers/:id/login",
  "POST /manage/teachers/:id/release",
  "POST /manage/years",
  "PUT /manage/grades/:id",
  "PUT /manage/periods/:id",
  "PUT /manage/subjects/:id",
  "PUT /manage/teachers/:id",
  "POST /owner/auth/logout",
  "GET /manage/press/orders/:id/file",
  "GET /owner/print-orders/counts",
  "GET /site/enquiries/:id",
  "POST /site/enquiries/:id/notes",
];

/**
 * Everything that predates this guard.
 *
 * MAY ONLY SHRINK. A new route belongs in `AUTHZ_REVIEWED`; putting one here
 * would defeat the entire point, which is that new code cannot slip through
 * unexamined. Moving a route out of this list means: decide who may call it,
 * write the assertion, move the string.
 */
export const AUTHZ_UNREVIEWED: string[] = [
  "DELETE /manage/class-notes/:id",
  "DELETE /manage/class-todos/:id",
  "DELETE /manage/diary/:id",
  "DELETE /manage/holidays/:id",
  "DELETE /manage/students/:id",
  "DELETE /owner/schools/:id",
  "GET /auth/me",
  "GET /directory",
  "GET /health",
  "GET /internal/cron/exam-reminders",
  "GET /manage/announcements",
  "GET /manage/attendance",
  "GET /manage/attendance/my-classes",
  "GET /manage/attendance/rates",
  "GET /manage/attendance/status",
  "GET /manage/class-log",
  "GET /manage/class-notes",
  "GET /manage/diary",
  "GET /manage/exams",
  "GET /manage/exams/:id/results",
  "GET /manage/holidays",
  "GET /manage/leave",
  "GET /manage/leave-policy/allocations",
  "GET /manage/leave-policy/my-balance",
  "GET /manage/leave-policy/pending-context",
  "GET /manage/leave-policy/types",
  "GET /manage/leave/coverage",
  "GET /manage/leave/mine",
  "GET /manage/messages",
  "GET /manage/messages/:threadId",
  "GET /manage/messages/unread-count",
  "GET /manage/note-classes",
  "GET /manage/register-changes",
  "GET /manage/register-changes/mine",
  "GET /manage/requests/pending-count",
  "GET /manage/school/class-note-visibility",
  "GET /manage/school/working-days",
  "GET /manage/staff-attendance/person",
  "GET /manage/students",
  "GET /manage/timetable/mine",
  "GET /marketing/blog",
  "GET /marketing/blog/:globalSlug",
  "GET /marketing/config",
  "GET /me/announcements",
  "GET /me/attendance",
  "GET /me/diary",
  "GET /me/exams",
  "GET /me/holidays",
  "GET /me/messages",
  "GET /me/messages/:threadId",
  "GET /me/messages/teachers",
  "GET /me/messages/unread-count",
  "GET /me/notifications",
  "GET /me/notifications/unread-count",
  "GET /me/profile",
  "GET /me/results",
  "GET /me/timetable",
  "GET /owner/blog/pending",
  "GET /owner/events",
  "GET /owner/jobs",
  "GET /owner/leads",
  "GET /owner/marketing-config",
  "GET /owner/ops",
  "GET /owner/overview",
  "GET /owner/schools",
  "GET /owner/schools/:id",
  "GET /owner/schools/:id/admins",
  "GET /owner/schools/:id/enquiries.csv",
  "GET /owner/stats",
  "GET /public/blog",
  "GET /public/blog/:slug",
  "GET /public/jobs",
  "GET /public/jobs/:id",
  "GET /public/site",
  "GET /ready",
  "GET /students",
  "GET /students/:id",
  "GET /teachers/:id",
  "GET /users",
  "GET /users/:id",
  "PATCH /manage/class-todos/:id",
  "PATCH /manage/diary/:id",
  "PATCH /manage/leave-policy/types/:id",
  "PATCH /owner/events/:id",
  "PATCH /owner/jobs/:id",
  "PATCH /owner/leads/:id",
  "PATCH /owner/schools/:id/features",
  "PATCH /owner/schools/:id/status",
  "PATCH /owner/schools/:id/tier",
  "POST /auth/accept-invite",
  "POST /auth/change-password",
  "POST /auth/forgot-password",
  "POST /auth/impersonate",
  "POST /auth/login",
  "POST /auth/logout",
  "POST /auth/refresh",
  "POST /auth/reset-by-code",
  "POST /auth/reset-password",
  "POST /auth/resolve-school",
  "POST /internal/cron/exam-reminders",
  "POST /manage/announcements",
  "POST /manage/attendance/notify-low",
  "POST /manage/class-notes",
  "POST /manage/class-todos",
  "POST /manage/diary",
  "POST /manage/exams",
  "POST /manage/exams/:id/publish",
  "POST /manage/holidays",
  "POST /manage/leave",
  "POST /manage/leave-policy/allocations/apply-defaults",
  "POST /manage/leave-policy/close-year",
  "POST /manage/leave-policy/types",
  "POST /manage/leave/:id/approve",
  "POST /manage/leave/:id/cancel",
  "POST /manage/leave/:id/reject",
  "POST /manage/messages/:threadId",
  "POST /manage/register-changes",
  "POST /manage/register-changes/:id/approve",
  "POST /manage/register-changes/:id/reject",
  "POST /manage/students",
  "POST /manage/students/:id/invite/resend",
  "POST /manage/students/:id/login",
  "POST /manage/substitution/:id/assign",
  "POST /manage/substitution/:id/clear",
  "POST /marketing/leads",
  "POST /me/diary/:id/sign",
  "POST /me/events/:id/register",
  "POST /me/messages",
  "POST /me/notifications/clear",
  "POST /me/notifications/read",
  "POST /me/photo",
  "POST /me/push-token",
  "POST /owner/auth/gate",
  "POST /owner/auth/login",
  "POST /owner/auth/refresh",
  "POST /owner/blog/:id/approve",
  "POST /owner/blog/:id/reject",
  "POST /owner/events",
  "POST /owner/schools",
  "POST /owner/schools/:id/admins/:userId/reset-password",
  "POST /owner/schools/:id/impersonate",
  "POST /public/enquiry",
  "POST /public/events/:id/register",
  "POST /public/jobs/:id/apply",
  "PUT /manage/attendance",
  "PUT /manage/exams/:id/results",
  "PUT /manage/leave-policy/allocations",
  "PUT /manage/school/class-note-visibility",
  "PUT /manage/school/working-days",
  "PUT /manage/students/:id",
  "PUT /owner/marketing-config",
];
