...
/**
 * Gallery, gated on the tier feature.
 *
 * Addressed by host as a route, not a header — middleware rewrites
 * <school>/gallery here and the visitor's URL is untouched. Reading the host
...
export default async function SchoolView({ params }: { params: Promise<{ host: string }> }) {
  ...
  if (!data.school.features.includes('GALLERY')) notFound();
  return <PublicSite data={data} view="gallery" />;
}
