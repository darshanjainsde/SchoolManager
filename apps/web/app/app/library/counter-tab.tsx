...
/**
 * The counter — the screen a librarian stands at all day.
 *
 * Ported from the standalone portal with its logic untouched. Every string an
 * assertion depends on ("Issue", "Issue anyway", "Available", "Holding N of M")
 * is deliberately unchanged: this was a re-home and a repaint, and a repaint
 * changes pixels, not words.
 */
export default function CounterTab() {
    ...
    <>
      ...
      {card.holdings.map((h) => (
        <div key={h.id} className="mt-1.5 flex items-center gap-2 text-xs text-[var(--sk-ink-2)]">
          ...
          {withReturn ? (
            <button className="sk-btn" data-size="sm" type="button" onClick={() => returnIssue.mutate(h.id)}>
              Return
            </button>
          ) : null}
        </div>
  ...
  return (
    <div>
      <div className="sk-seg mb-4" style={{ maxWidth: 320 }} role="group" aria-label="Counter mode">
        {(
          [
            ['out', 'Give out'],
            ['back', 'Take back'],
      ...
      {mode === 'out' ? (
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <ScanBox
              value={memberQ}
              onChange={(v) => { setMemberQ(v); setSelMember(null); }}
              placeholder="Student ID (AAA-00000), name, or a teacher…"
              label="Find a reader"
            />
            {!selMember && memberHits.data ? (
              <Card className="mt-2 overflow-hidden">
                {memberHits.data.length ? (
                  ...
                  <div className="px-4 py-4 text-center"><EmptyRow>No reader matches “{dMemberQ.trim()}”.</EmptyRow></div>
                )}
              </Card>
            ) : null}
            {selMember && memberCard.data ? (
              <Card className="relative mt-2 p-3">
                <button
                  className="sk-btn absolute right-2 top-2"
                  data-size="sm"
                  type="button"
                  aria-label="Change reader"
                  onClick={() => { setSelMember(null); setMemberQ(''); setWarn(null); setLastIssue(null); }}
                >
                  Change
                </button>
                <div className="pr-20 text-sm font-bold text-[var(--sk-ink)]">{memberCard.data.borrower.name}</div>
          ...
          <div>
            ...
            <ScanBox
              value={bookQ}
              onChange={(v) => { setBookQ(v); setSelTitleId(null); }}
              placeholder="Title, author or book number…"
              label="Find a book"
            />
              ...
              <Card className="mt-2 overflow-hidden">
                {titleHits.data.length ? (
                  ...
                  <div className="px-4 py-4 text-center">
                    <EmptyRow>No title matches “{dBookQ.trim()}” — add it under Catalogue.</EmptyRow>
                  </div>
                )}
              </Card>
            ) : null}
            {selTitleId && titleCard.data ? (
              <Card className="relative mt-2 p-3">
                ...
                <div className="pr-20 text-sm font-bold text-[var(--sk-ink)]" style={{ fontFamily: 'var(--sk-serif)' }}>
                ...
                {warn ? (
                  <div className="mt-2 rounded-lg border border-[var(--sk-amber)] bg-[var(--sk-amber-tint)] p-2.5 text-xs text-[var(--sk-amber-ink)]">
                    <span aria-hidden="true">⚠️</span> {warn}
                  ...
                  <button
                    className="sk-btn mt-2.5 w-full"
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <ScanBox
              value={retQ}
              onChange={(v) => { setRetQ(v); setRetMember(null); }}
              placeholder="Student ID, name, title or book number…"
              label="Find a loan"
            />
            ...
            {retMember && retMemberCard.data ? (
              <Card className="relative mt-2 p-3">
                ...
                <div className="pr-20 text-sm font-bold">{retMemberCard.data.borrower.name}</div>
            ...
            {retResult ? (
              <Card className="relative mt-3 p-3" data-testid="return-card">
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {!retResult.issue.wasLost ? (
                    <>
                      <button className="sk-btn" data-size="sm" type="button" onClick={() => reopen.mutate(retResult.issue.id)}>
                        Undo return
                      </button>
                      <button className="sk-btn" data-size="sm" type="button" onClick={() => markLost.mutate(retResult.issue.id)}>
                ...
                {dash.data?.outNow.length ? (
                  dash.data.outNow.slice(0, 10).map((i) => (
                    ...
                    >
                      ...
                      <button className="sk-btn" data-size="sm" type="button" onClick={() => returnIssue.mutate(i.id)}>
                        Return
                      </button>
                    </ListRow>
                  ))
