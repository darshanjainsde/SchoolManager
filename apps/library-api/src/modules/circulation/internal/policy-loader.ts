...
/**
 * Loads the `CirculationPolicy` row for `(orgId, memberType)` and converts
 * it into `policy.ts`'s pure `Policy` shape — the ONLY place this
 * Decimal-to-number conversion happens, so `evaluateIssue`/`evaluateRenew`/
 * `computeFine` never see a Prisma `Decimal` and every circulation call site
 * gets numbers built the same way. Read on `tx`, so it is scoped by the same
 * `withOrg` transaction (and therefore RLS) as the caller's other reads.
 *
 ...
 */
export async function loadPolicy(tx: LibraryTx, orgId: string, memberType: MemberType): Promise<Policy> {
  const row = await tx.circulationPolicy.findUnique({ where: { orgId_memberType: { orgId, memberType } } });
  if (!row) {
