 ...
 *
 * Moved verbatim from the old `more.tsx` (removed with the More tab) so the
 * login/logout call shape is unchanged.
 */
...
/**
 * Bottom-sheet tools drawer (pitch: Phone 3). A dimming scrim + a rounded
 * sheet that slides up from the bottom over whatever tab is showing, laying
 * out every `MORE_ITEMS` tool in a 3-column grid with live badges, plus Log
 * out. The chevron FAB in `StaffTabBar` toggles `open`; the parent
