...
export interface LibraryMe {
  ...
  today: string;
  /** The school's rules, so the shelf can state them rather than let a fine teach them. Optional: an older API has none. */
      ...
      {d && (
        <>
          <View style={{ flexDirection: 'row', gap: 8 }}>
            ...
            />
            {d.finesEnabled ? (
              <Figure testID="library-fine" label="Fine" value={fine(d.finesDueRupees)} hint={d.finesDueRupees > 0 ? 'pay at the counter' : d.rules ? `nothing owed · ${fine(d.rules.finePerDayRupees)} a day late` : 'nothing owed'} tone={d.finesDueRupees > 0 ? 'bad' : undefined} />
            ) : (
