...
it('renders the sheet with every MORE_ITEMS tool in the grid when open', () => {
  const { getByTestId, getByText } = render(<FamilyToolsDrawer open onClose={jest.fn()} />);
...
it('renders nothing while closed (no overlay to intercept touches)', () => {
  const { queryByTestId } = render(<FamilyToolsDrawer open={false} onClose={jest.fn()} />);
  expect(queryByTestId('tools-sheet')).toBeNull();
  expect(queryByTestId('tools-scrim')).toBeNull();
});

it('shows a LIVE badge from the unread-count endpoint on the Messages tile', async () => {
  // Messages → /me/messages/unread-count, fetched when the drawer opens.
  // Notices has no count endpoint, so it never carries a badge.
  mockFetch.mockImplementation(async (url: unknown) => ({
    ok: true,
    status: 200,
    json: async () => ({
      count: String(url).includes('/me/messages/unread-count') ? 3 : 0,
    }),
  }));
  const { getByTestId, getByText, queryByTestId } = render(<FamilyToolsDrawer open onClose={jest.fn()} />);
  await waitFor(() => expect(getByTestId('tool-badge-Messages')).toBeTruthy());
  expect(getByText('3')).toBeTruthy();
  // No count endpoint exists for these → never a badge.
  expect(queryByTestId('tool-badge-Notices')).toBeNull();
  expect(queryByTestId('tool-badge-Assignments')).toBeNull();
});

it('shows NO badge on the Messages tile when its count is zero', async () => {
  mockFetch.mockImplementation(async () => ({ ok: true, status: 200, json: async () => ({ count: 0 }) }));
  const { queryByTestId } = render(<FamilyToolsDrawer open onClose={jest.fn()} />);
...
it('tapping a tool navigates to its route and closes the drawer', () => {
  ...
  const { getByTestId } = render(<FamilyToolsDrawer open onClose={onClose} />);

  fireEvent.press(getByTestId('tool-Timetable'));

  expect(mockPush).toHaveBeenCalledWith('/(family)/timetable');
  expect(onClose).toHaveBeenCalledTimes(1);
});

it('tapping the Notices tool navigates to the notices screen (moved out of the tab bar)', () => {
  const onClose = jest.fn();
  const { getByTestId } = render(<FamilyToolsDrawer open onClose={onClose} />);

  fireEvent.press(getByTestId('tool-Notices'));
...
it('Log out revokes the token, clears the session and routes to connect', async () => {
  ...
  // The drawer also fires the badge-count fetch on open, so find the logout
  // call among them rather than assuming it is the only/first one.
  await waitFor(() =>
...
it('still clears the session and navigates when the logout POST fails (offline)', async () => {
  ...
  const { getByTestId } = render(<FamilyToolsDrawer open onClose={jest.fn()} />);
