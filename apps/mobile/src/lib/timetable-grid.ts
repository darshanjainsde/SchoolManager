/**
 * Pure shaping helpers for the teacher's weekly timetable. No React, no
 * `Date` — the screen reads the clock and passes plain numbers/ids down;
 * this module only ever sees data it's given, so it's testable without
 * rendering or faking time.
 *
 * Ported from apps/web/lib/timetable-grid.ts. Same semantics, deliberately —
 * the phone and the browser must never disagree about which period is
 * "current" or which slot wins a collision. Shape differs (the web keeps a
 * `Map` of every day:period cell for a table; the phone only ever renders
 * one selected day, so its screen reads `cells` via `cellKey` directly
 * rather than needing a second view model) but `buildGrid`'s output is
 * identical, byte for byte, so both surfaces can be tested against the same
 * fixtures.
 */
